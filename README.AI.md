...
This docuement generated by the [readme.py](summarisers/readme.py) script.
        f"21 Python solutions were analysed.
...
- **Problem 2**: If removing a single level from an invalid report would make it valid, count that report as valid.
...
- **Solution 2**: Checks if the original report is valid. If not, it iterates through the report, removing one level at a time and checking if the remaining report is valid. Counts the number of valid reports, including those made valid by removing a level.
...
- **Solution 1**: Parses the input equations, then recursively tries all possible combinations of addition and multiplication operators on the numbers to see if any combination equals the target value.
- **Solution 2**: Similar to Part 1, but adds the concatenation operator to the list of allowed operators when recursively trying combinations.
...
- **Problem 2**: Calculate the sum of ratings for all trailheads, where a trailhead's rating is the number of distinct hiking trails that begin at that trailhead. A hiking trail is a sequence of points with increasing heights from 0 to 9.
- **Solution 1**: Implements a recursive function trailhead_destinations() that finds all destinations (height 9) reachable from a starting point by following increasing heights. Iterates over all trailheads, calls this function, and sums the lengths of the destination sets.
- **Solution 2**: Implements a recursive function complete_trails() that finds all complete trails (length 18) starting from a given point by following increasing heights. Iterates over all trailheads, calls this function, and sums the lengths of the trail sets.
...
- **Problem 1**: Find the lowest score a Reindeer could get to navigate through a maze from the start tile to the end tile. The Reindeer can move forward one tile at a time or rotate 90 degrees clockwise or counterclockwise, incurring penalties.
- **Problem 2**: Determine how many tiles are part of at least one of the best paths through the maze.
- **Solution 1**: Uses A* search (with Manhattan distance heuristic) or Uniform Cost Search to find the least costly path(s) through the maze. Reconstructs all optimal paths from the predecessors dictionary.
- **Solution 2**: Reuses the optimal path(s) found in Part 1 to identify all tiles that are part of any optimal path.
...
- **Solution 1**: Creates a 2D grid representing the memory space. Marks the coordinates of the falling bytes as walls. Uses Uniform Cost Search with a priority queue to find the shortest path from (0,0) to (70,70), avoiding walls.
- **Solution 2**: Starts with the path found in Part 1. Iterates through the remaining bytes, blocking each one and checking if a path to the exit still exists using Uniform Cost Search. Returns the coordinates of the first byte that blocks the path.
...
- **Solution 2**: Extends the depth-first search from Part 1 to not only check if a design is possible, but also count the number of different ways it can be constructed by combining towel patterns.
