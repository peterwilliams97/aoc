"""
    This is a critic that produces validated structured critiques of Python solutions to
    Advent of Code problems using a Pydantic data model.

    Each Advent of Code Python solution file has:
    - A summary of the problem in a comment at the top of the file.
    - A solution to PART 1 in a function called part1.
    - A solution to PART 2 in a function called part2.
    - A call to parse_args to parse the command line arguments.
    - A call to the part1 and part2 function with the parsed arguments.

    The structured critique includes the following fields:
    - Name: The name of the Python file. (string)
    - Description: The description of the problem from the comment at the top of the file. (string)
    - Problem1: The summary of the PART 1 problem. (string)
    - Problem2: The summary of the PART 2 problem. (string)
    - Solution1: The solution to PART 1. (string)
    - Solution2: The solution to PART 2. (string)
    - Strengths: The strengths of the programming and problem solving. (list of strings)
    - Weaknesses: The weaknesses of the programming and problem solving. (list of strings)
    - Issues: The function names where inefficient code, code duplication, hardcoded
    values or fragile code occurs. (list of strings)
    - Overall: The overall assessment of the solution. (string)
    - Score: A score in the range 1 to 10 where 1 is beginner level, 5 is an average solution and 10
    is the top of Advent of Code solutions. (int)


    Here's a critique of the provided Advent of Code 2024 Day 16 solution:

    Name: aoc2024-day16.py

    Description: --- Day 16: Reindeer Maze ---

    Problem1:

    In Part 1 of the Reindeer Maze problem, the objective is to determine the lowest score a
    Reindeer can achieve while navigating from the start tile ('S') to the end tile ('E') in a maze.
    The Reindeer can move forward one tile at a time, increasing their score by 1 point per step,
    and can rotate 90 degrees clockwise or counterclockwise, increasing their score by 1000 points
    per turn. The challenge is to find the optimal path that minimizes the total score, considering
    both the number of steps and the number of turns required to reach the end tile without moving
    into walls ('#').

    Problem2:

    In Part 2 of the Reindeer Maze problem, the goal is to identify all the tiles that are part of
    any best path through the maze, including the start ('S') and end ('E') tiles. After determining
    the lowest score paths in Part 1, you need to analyze the maze to find which non-wall tiles
    (S, ., or E) are included in at least one of these optimal paths. This information is crucial
    for determining the best spots to sit and watch the action, as sitting on these tiles ensures
    you won't miss any of the Reindeer's movements. The problem requires you to mark all such tiles
    on the maze, highlighting the paths that contribute to the lowest score.

    Solution1:

    The part1() function in the aoc2024-day16.py script aims to solve the Reindeer Maze problem by
    finding the lowest score a Reindeer can achieve while navigating from the start ('S') to the end
    ('E') tile in a maze. The function utilizes the solve_maze() function, which implements an
    A* search algorithm to explore possible paths through the maze. The algorithm considers both the
    number of steps and the number of turns, assigning a penalty of 1 point per step and 1000 points
    per turn. The solve_maze() function tracks multiple paths and reconstructs all paths that
    achieve the minimum score, ensuring that all optimal paths are identified. The part1()
    function then prints the minimum score and the best paths found. While the function effectively
    finds the optimal paths, it could benefit from additional comments and error handling to improve
    readability and robustness. Additionally, the function could be optimized further to handle
    larger mazes more efficiently.

    Solution2:

    The part2() function in the aoc2024-day16.py script extends the solution from Part 1 by
    identifying all tiles that are part of any best path through the maze, including the start ('S')
    and end ('E') tiles. After determining the lowest score paths using the solve_maze() function,
    which employs an A* search algorithm, part2() analyzes the maze to find which non-wall tiles
    (S, ., or E) are included in at least one of these optimal paths. The function effectively
    tracks multiple predecessors for each state to ensure all possible paths are considered, even
    when paths split and rejoin. It then reconstructs all valid paths from the end to the start,
    marking the tiles that are part of these paths. While the function successfully identifies and
    marks all relevant tiles, it could benefit from additional comments and error handling to
    enhance readability and robustness. Furthermore, optimizing the path reconstruction process
    could improve performance, especially for larger mazes

    Strengths:

    1 Well-organized structure with separate functions for different parts of the solution.
    2 Good use of comments to explain the purpose of functions and key parts of the code.
    3 Efficient use of A* search algorithm for pathfinding.
    4 Clever use of a heuristic function to optimize the search.
    5 Handling of multiple optimal paths, not just finding a single solution.
    6 Use of heapq for efficient priority queue implementation.
    7 Clear separation of Part 1 and Part 2 solutions.
    8 Inclusion of timing information for performance analysis.

    Weaknesses:

    1 The solve_maze function is quite long and could potentially be split into smaller, more focused functions.
    2 Some magic numbers are used (e.g., PENALTY_MOVE = 1, PENALTY_TURN = 1000) which could be defined as constants at the top of the file for better
    maintainability.
    3 The reconstruct_paths function uses recursion, which could potentially lead to stack overflow for very large mazes.
    4 The solution doesn't handle invalid input gracefully (e.g., missing start or end points).
    5 There's some code duplication in the handling of predecessors and visited states.

    Issues:

    1 In solve_maze: The handling of multiple paths and predecessors could potentially be memory-intensive for very large mazes.
    2 In reconstruct_paths: The recursive approach could be inefficient for very long paths.
    3 In part2: Creating a set of all tiles in all best paths could be memory-intensive for large mazes with many optimal paths.

    Overall:

    This solution demonstrates a strong understanding of algorithmic problem-solving and efficient implementation. The use of A* search with a heuristic function is particularly impressive. The
    code is well-structured and commented, making it easy to understand and maintain.

    The main areas for improvement are in code organization (breaking down the solve_maze function),
    handling edge cases, and potentially optimizing memory usage for very large inputs. However,
    these are relatively minor points in the context of an Advent of Code solution.

    The solution goes beyond simply solving the problem by handling multiple optimal paths and
    providing timing information, which shows a high level of thoroughness and attention to detail. -
...
CRITIQUE_SYSTEM_PROMPT = f"""
    ...
    {coding_criteria}
    ---------------------------
    Scores range from 1 to 10, where 1 is beginner level, 5 is an average solution, and 10 is the
    top of Advent of Code solutions.
